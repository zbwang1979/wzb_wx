...
from zbw_scraber_super import zbw_scraber_instagram
import os
import emoji
...
#down_loader callback
def send_files(name,desc,file_type,owner_name=None):
    chat_list=[]
    chat_list.append(ensure_one(bot.groups().search(puid=dict_puid['名称='])))
    tz = pytz.timezone(pytz.country_timezones('cn')[0])
    now_time = datetime.datetime.now(tz)
    if now_time.hour==12 or now_time.hour>=18:
        chat_list.append(ensure_one(bot.groups().search(puid=dict_puid['技术'])))
    # list.append(bot.file_helper)
    i=0
    for item in chat_list:
        i+=1
        txt='from ins%s%s:\n%s'%(''.join(['#' for j in list(range(i))]),owner_name,desc)
        item.send_msg(txt)
        if 'image' in file_type.lower():
            item.send_image(name)
        else:
            item.send_video(name)
...
#down_loader
ins = zbw_scraber_instagram('xxxxxxxxxxxxxxx', 'xxxxxxxxxxx', call_back=send_files)
def new_scheduled():
        ...
        def scheduled_job():
            ...
            mp.send('北京')
            my_print('job_send')
        @sched.scheduled_job('cron', id='ins_downloader', hour='8-21/2', minute='15')
        def scheduled_job():
            time.sleep(1)
            ins.run_flow()
        sched.start()
t = threading.Thread(target =new_scheduled,daemon=True)
t.start()
